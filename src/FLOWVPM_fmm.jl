...
function fmm.direct!(target_system, target_index, source_system::ParticleField, source_index)
    ...
    elseif source_system.toggle_sfs
        Estr_direct(target_system, target_index, source_system, source_index, source_system.transposed)
    else
        for j_target in target_index
            target_x, target_y, target_z = target_system[j_target,fmm.POSITION]
            for i_source in source_index
                ...
                if !iszero(r2)
                    ...
                    du3x3 = aux * crss3 * dz
                    vec(target_system[j_target,fmm.VELOCITY_GRADIENT]) .+= du1x1, du2x1, du3x1, du1x2, du2x2, du3x2, du1x3, du2x3, du3x3
                end
            end
        end
    end
