...
function UJ_direct(dpfield, pfield, g_dgdr::Function,np)

  plen = Int((length(pfield))/np)
  for i=1:np
    for j=1:np

      i0 = (i-1)*plen
      j0 = (j-1)*plen
      _pi = pfield[i0+1:i0+plen]
      _dpi = dpfield[i0+1:i0+plen]
      _pj = pfield[j0+1:j0+plen]
      #dX1 = Pi.X[1] - Pj.X[1]
      #dX2 = Pi.X[2] - Pj.X[2]
      #dX3 = Pi.X[3] - Pj.X[3]
      dX1 = get_X(_pi)[1]
      ...
      if r2!=0
       ...
       r = sqrt(r2)
        # Regularizing function and deriv
        g_sgm, dg_sgmdr = g_dgdr(r/get_sigma(_pj)[1])

        # K × Γp
        #crss1 = -const4 / r^3 * ( dX2*Pj.Gamma[3] - dX3*Pj.Gamma[2] )
        #crss2 = -const4 / r^3 * ( dX3*Pj.Gamma[1] - dX1*Pj.Gamma[3] )
        #crss3 = -const4 / r^3 * ( dX1*Pj.Gamma[2] - dX2*Pj.Gamma[1] )
        crss1 = -const4 / r^3 * ( dX2*get_Gamma(_pj)[3] - dX3*get_Gamma(_pj)[2] )
        crss2 = -const4 / r^3 * ( dX3*get_Gamma(_pj)[1] - dX1*get_Gamma(_pj)[3] )
        crss3 = -const4 / r^3 * ( dX1*get_Gamma(_pj)[2] - dX2*get_Gamma(_pj)[1] )

        # U = ∑g_σ(x-xp) * K(x-xp) × Γp
        #Pi.U[1] += g_sgm * crss1
        #Pi.U[2] += g_sgm * crss2
        #Pi.U[3] += g_sgm * crss3
        get_U(_dpi)[1] = g_sgm * crss1
        get_U(_dpi)[2] = g_sgm * crss2
        get_U(_dpi)[3] = g_sgm * crss3

        # ∂u∂xj(x) = ∑[ ∂gσ∂xj(x−xp) * K(x−xp)×Γp + gσ(x−xp) * ∂K∂xj(x−xp)×Γp ]
        # ∂u∂xj(x) = ∑p[(Δxj∂gσ∂r/(σr) − 3Δxjgσ/r^2) K(Δx)×Γp
        aux = dg_sgmdr/(get_sigma(_pj)[1]*r) - 3*g_sgm /r^2
        #=
        # j=1
        Pi.J[1, 1] += aux * crss1 * dX1
        Pi.J[2, 1] += aux * crss2 * dX1
        ...
        =#
        # j=1
        get_J(_pi)[1] += aux * crss1 * dX1
        get_J(_pi)[2] += aux * crss2 * dX1
        get_J(_pi)[3] += aux * crss3 * dX1
        # j=2
        get_J(_pi)[4] += aux * crss1 * dX2
        get_J(_pi)[5] += aux * crss2 * dX2
        get_J(_pi)[6] += aux * crss3 * dX2
        # j=3
        get_J(_pi)[7] += aux * crss1 * dX3
        get_J(_pi)[8] += aux * crss2 * dX3
        get_J(_pi)[9] += aux * crss3 * dX3
        ...
        =#
        # j=1
        get_J(_pi)[2] -= aux * get_Gamma(_pj)[3]
        get_J(_pi)[3] += aux * get_Gamma(_pj)[2]
        # j=2
        get_J(_pi)[4] += aux * get_Gamma(_pj)[3]
        get_J(_pi)[6] -= aux * get_Gamma(_pj)[1]
        # j=3
        get_J(_pi)[7] -= aux * get_Gamma(_pj)[2]
        get_J(_pi)[8] += aux * get_Gamma(_pj)[1]
