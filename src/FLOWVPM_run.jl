   ...
   #= (line1, line2, run_id, file_verbose,
    ...
    if mode == "forwards" || mode == "adjoint"
        ...
        p = SolverSettings{Float64}(;save_parameters = save_parameters,
                            new_particle_function=new_particle_function,
                            new_particle_function_parameters=new_particle_function_parameters,
                            other_function=other_function,
                            other_function_parameters=other_function_parameters,
                            kernel=pfield.kernel,UJ=pfield.UJ,
                            formulation=pfield.formulation,
                            viscous=pfield.viscous,
                            Uinf=pfield.Uinf,
        ...
        # Third: New particle function callback
        if new_particle_function !== nothing
            cb3 = PresetTimeCallback(new_particle_times,new_particle_function)
        else
            cb3 = DiscreteCallback(FalseCondition,NullAffect!)
        end
        # Fourth: Generic user-defined function callback
        if other_function !== nothing
            cb4 = DiscreteCallback(TrueCondition,other_function)
        else
            cb4 = DiscreteCallback(FalseCondition,NullAffect!)
        end
        ...
        if mode == "forwards"
            ...
            sol = diffeq.solve(prob,p.integration,callback=cbs;dt=dt,save_on=false,alias_u0=true) # Note: save_on=false disables saving at all intermediate steps in RAM, but still causes output to be written to disk.
        elseif mode == "adjoint"
            ...
            prob = diffeq.ODEProblem(DiffEQ_derivative_function!,u0,tspan,p)
            tracked_cbs = disadj.setup_tracked_callback(CallbackSet(cbs),u0,p,tspan[1])
            sol = diffeq.solve(prob,p.integration,callback=tracked_cbs;dt=dt,alias_u0=true,save_on=true)
            ts = sol.t

            dp,du0 = DiscreteAdjoint.discrete_adjoint(sol,dZ_discrete,ts;cb=tracked_cbs)#,autojacvec = disadj.ReverseDiffVJP(false))
        end
        # Remove static particles (assumes particles remained sorted)
        ### TEMP: Disabled
