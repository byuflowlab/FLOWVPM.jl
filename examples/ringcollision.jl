...
"""
Validation on vortex stretching, mixing, and reconnection by simulating the
interaction collision of two vortex rings. Compare with the experimental
...
function validation_ringcollision(;   kernel=vpm.kernel_wnklmns, UJ=vpm.UJ_fmm,
    ...
    R1, R2 = 33e-3/2, 33e-3/2
    Gamma1, Gamma2 = -0.0001*16, -0.0001*16

    if nc==0
      sep_factor = 2.0
      rmaxfactor1, rmaxfactor2 = 1/15*2, 1/15*2
      lambda1, lambda2 = 1.6, 1.6
    else
      sep_factor = 1.0
      rmaxfactor1, rmaxfactor2 = 1/15, 1/15
      lambda1, lambda2 = 1.9, 1.9
    end
    angle1, angle2 = 14.0*pi/180, -14.0*pi/180   # (rad) inclination of rings

    coord_sys1 = [cos(angle1) 0 sin(angle1); 0 1 0; -sin(angle1) 0 cos(angle1)]
    coord_sys2 = [cos(angle2) 0 sin(angle2); 0 1 0; -sin(angle2) 0 cos(angle2)]

    rmax1, rmax2 = R1*rmaxfactor1, R2*rmaxfactor2
    C1 = [-(R1+rmax1*(1+sep_factor))*cos(angle1), 0, 0]
    C2 = [(R2+rmax2*(1+sep_factor))*cos(angle2), 0, 0]

    sigma1 = lambda1*(2*pi*R1/Nphi)
    sigma2 = lambda2*(2*pi*R2/Nphi)
    coR1 = sigma1/R1
    coR2 = sigma2/R2

    Re = Gamma1/nu

    run_ringcollision(R1, R2, Gamma1, Gamma2, coR1, coR2,
                                  Nphi, nc, Re,
                                  nsteps, nR;
                                  extra_nc=extra_nc,
                                  faux1=faux1,
...
"""
function run_ringcollision(R1::Real, R2::Real,
