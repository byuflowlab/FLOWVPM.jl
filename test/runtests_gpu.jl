...
#
# NOTE: this deliberately does not build particle fields directly on GPU via
# add_particle -- add_particle's per-particle setters do scalar CuArray
# indexing, which CUDA.jl disallows (the same bug class fixed in
# _reset_particles/_reset_particles_sfs). Fixing add_particle for GPU use is
# out of scope here; instead, fields are built on CPU and their underlying
# matrix is copied wholesale onto the GPU, matching how gpu-full's own
# hardware validation was done.
using Test
import Random
vpm = FLOWVPM
