#!/bin/bash
#SBATCH --job-name=fp052cpu-pinned
#SBATCH --cpus-per-task=64
#SBATCH --mem=192G
#SBATCH --time=08:00:00
#SBATCH --requeue
#SBATCH --output=%x-%j.out
# Mature CPU-S continuation from the protected step-719 checkpoint with the
# wake FMM PINNED (p=4, ncrit=50, theta=0.4, autotune off) — the 052a
# option-(d) deterministic reference regeneration. Requeue-tolerant: the run
# dir is wiped at start because the driver APPENDS to existing PVD
# collections, so a requeued attempt must not inherit partial output.
source /etc/profile
set -euo pipefail
...
RUN="${FP052_RUN_NAME:-fm052r_cpu_mature_pinned}"
rm -rf "data/$RUN" "data/${RUN}.log"
mkdir -p "data/$RUN"
